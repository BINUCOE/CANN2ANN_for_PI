...
class YawHeightHDCNetwork:
    def __init__(self, **kwargs):
        # The dimension and range of yaw in yaw_height_hdc network
        ...
        self.YAW_HEIGHT_HDC_H_RANGE = kwargs.pop('YAW_HEIGHT_HDC_H_RANGE', 36)
        ...
        #The real resolution of each unit:(range[1] - range[0]) / dim
    ...
    def get_current_yaw_height_value(self):
        ...
        y, h = np.unravel_index(self.YAW_HEIGHT_HDC.argmax(), self.YAW_HEIGHT_HDC.shape, order='C')
        ...
        tempYawHeightHdc[np.ix_(self.YAW_HEIGHT_HDC_MAX_Y_WRAP[y: y + self.YAW_HEIGHT_HDC_PACKET_SIZE * 2 + 1],
            ...
            self.YAW_HEIGHT_HDC[np.ix_(self.YAW_HEIGHT_HDC_MAX_Y_WRAP[y: y + self.YAW_HEIGHT_HDC_PACKET_SIZE * 2 + 1],
                                self.YAW_HEIGHT_HDC_MAX_H_WRAP[h: h + self.YAW_HEIGHT_HDC_PACKET_SIZE * 2 + 1])]
        # self.yaw_state.append(np.sum(tempYawHeightHdc,axis = 1))
    ...
    def path_integration(self, yawRotV, heightV,):
        if yawRotV != 0:
            weight = np.mod(np.abs(yawRotV) / self.YAW_HEIGHT_HDC_Y_RES, 1)
            if weight == 0:
                weight = 1.0

            shift1 = int(np.sign(yawRotV) * np.floor(
                np.mod(np.abs(yawRotV) / self.YAW_HEIGHT_HDC_Y_RES, self.YAW_HEIGHT_HDC_Y_DIM)))
        ...
        if heightV != 0:
            weight = np.mod(np.abs(heightV) / self.YAW_HEIGHT_HDC_H_RES, 1)
            if weight == 0:
                weight = 1.0
            shift1 = int(np.sign(heightV) * np.floor(
                np.mod(np.abs(heightV) / self.YAW_HEIGHT_HDC_H_RES, self.YAW_HEIGHT_HDC_H_DIM)))
    ...
    def yaw_height_hdc_iteration(self, vt_id, yawRotV, heightV, VT):
        ...
        if VT[vt_id].first != 1:
            ...
            print(VT[vt_id].hdc_yaw, VT[vt_id].hdc_height, energy)
            self.lcd_num += 1
            # self.vt_history.append([energy, act_yaw, act_height])
        # else: self.vt_history.append([0, 0, 0])
        ...
        self.path_integration(yawRotV, heightV)

        self.MAX_ACTIVE_YAW_HEIGHT_HIS_PATH = list(self.get_current_yaw_height_value())
