import torch
import glob
from src.func import cann3d_pre_encoder,get_data
from torch.optim import lr_scheduler
import os
import random
from torch.utils.data import Dataset, DataLoader

#nums = dims**3,equl to mixed CANN ,37^3->flatten
class CANNDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cur_data = torch.tensor((cur_data))/4000
        I = torch.from_numpy(data_list[0])
        # print(self.file_list[idx], cur_data.shape)
        return I, cur_data
class CANN3Dnet(torch.nn.Module):
    ...
    def compute_loss(self , net_out, cann_out, out_id ):
        loss1 = torch.nn.MSELoss(reduction="sum")(net_out[out_id], cann_out)
        return loss1
...
def train(net, num_epochs ,out_id, save_dir ,datadir, trainer_list, scheduler_list,weights_name,batch_size = 10,val_split = 0.2):
    ...
    for epoch in range(num_epochs):
        ...
        for I, cann_out_batch in train_loader:
            ...
            #将模拟输入预编码后输入到神经网络中
            I_encoded = cann3d_pre_encoder(I ,net.nums, concat = True)
        ...
        with torch.no_grad():
            for I_val, cann_out_val_batch in val_loader:
                ...
                # 将模拟输入预编码后输入到神经网络中
                I_val_encoded = cann3d_pre_encoder(I_val,net.nums, concat=True)
                I_val_encoded = I_val_encoded.reshape(cann_out_val_batch.shape[0] , cann_out_val_batch.shape[1], I_val_encoded.shape[1])
...
if __name__ == "__main__":
    save_dir = "../temp"
    datadir = "../dataset_3d_49"
    weights_name = "model3d_weights_49.pth"
    mode = 1
    out_id = 2
